...
<html>
	<head>
		...
		<![endif]-->
		<title>晚9点未结束订单</title>
	</head>
	<body>
		<nav class="breadcrumb">
			<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>订单管理 <span class="c-gray en">&gt;</span> 晚9点未结束订单
		...
		<script type="text/javascript">
			...
			function operatBtnContro() {
				$("#data-list-table").find("tr").each(function(index) {
					if (index != 0) {
						...
						if (status == 5 || status == 6 || status == 11 || status == 12) {
							$(this).find("a[name='order-jy']").show();
						} else {
							$(this).find("a[name='order-jy']").hide();
						}
					}
				});
			} 
			 ...
			 * 列表数据、分页绑定
			 */
			var queryRowUrl  = "/rest/um/order/orderNineCount";
			var queryDataUrl = "/rest/um/order/orderNineList";
			...
			var operatHtml = "";
				...
				operatHtml += "<a class=\"ml-10\" style=\"color:#0A6999\" title=\"查看金鹰位置\" href=\"javascript:;\" name=\"order-jy\" dataKey=\"id\" dataId=\"operatId\"><i class=\"Hui-iconfont Hui-iconfont-dingwei\"></i>坐标</a>";
			...
			var orderJyWindow = {};
			orderStrokeWindow.domName = "order-jy";
			orderStrokeWindow.title = "查看金鹰位置";
			orderStrokeWindow.type = "confirm";
			orderStrokeWindow.url   = "/rest/um/order/toTravelDetails";
			orderStrokeWindow.type = "full";
