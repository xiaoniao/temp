...
<html>
	...
	<body>
		...
		<div class="page-container">
			...
			<div class="text-c" style="margin-top: 10px;">
				...
				<span class="select-box inline" style="padding:0">
					<select name="payMethod" id="payMethod" class="select input-text help-inline">
						<option value="">-- 请选择资金来源 --</option>
						<option value="0">预存款支付</option>
		...
		<script type="text/javascript">
			...
			* 列表数据、分页绑定
			*/
			var queryRowUrl  = "/rest/um/money/recharge/listPageCount";
			var queryDataUrl = "/rest/um/money/recharge/listPage";
			...
			var dataListKey = [
				...
				{key:'payMethodStr', type:'text'},
				{key:"statusStr", type:'text'},
				{key:"tradeNo", type:'text'}
			];

			var pagesize = 12;
			var searchInput = ['bankMobile', 'starDate', 'endDate', 'payMethod', 'status', 'tradeNo'];
			var operatHtml = "";
			var operatWindows = [];
			...
			function excel() {
				...
				$.ajax({
					type : "post",
					dataType : "JSON",
					url : "/rest/um/money/recharge/listAll",
					data : null,
					success : function(result) {
						if (result.statusCode == 0) {
							/* original data */
							var data = [];
							data.push(['编号','会员账号','金额','时间','资金来源','充值状态','交易号']);
							for(var i = 0; i < result.data.length; i++) {
								var item = [];
								item.push(i + 1);
								item.push(result.data[i].bankMobile);
								item.push(result.data[i].money);
								item.push(result.data[i].createTimeStr);
								item.push(result.data[i].payMethodStr);
