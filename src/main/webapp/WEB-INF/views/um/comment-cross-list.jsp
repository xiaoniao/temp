...
<html>
	...
	<body>
		<nav class="breadcrumb">
			<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>对金鹰的评价
		...
		<div class="page-container">
			...
			<div class="text-c" style="margin-top: 10px;">
				...
				<input type="text" name="workName" id="workName" placeholder="金鹰工作名" style="width:120px" class="input-text">
		...
		<script type="text/javascript">
			...
			* 列表数据、分页绑定
			*/
			var queryRowUrl  = "/rest/um/comment/cross/listPageCount";
			var queryDataUrl = "/rest/um/comment/cross/listPage";
			...
			var pagesize = 12;
			var searchInput = ['cityId', 'bankMobile', 'starDate', 'endDate', 'workName'];
			var operatHtml = "";
				operatHtml += "<a id=\"btnoperatId\" class=\"ml-10 btn btn-primary\" title=\"通过审核\" href=\"javascript:;\" name=\"comment-operate-btn\" dataKey=\"id\" dataId=\"operatId\" onclick=operate(operatId)>审核通过</a>";
				operatHtml += "<a id=\"labeloperatId\" class=\"label label-success\" name=\"comment-operate-label\">已审核</a>";
				...
				$("#data-list-table").find("tr").each(function(index) {
					if (index != 0) {
						...
						if (status != null && status == 1) {
							$(this).find("a[name='comment-operate-btn']").hide();
							$(this).find("a[name='comment-operate-label']").show();
						} else {
							$(this).find("a[name='comment-operate-btn']").show();
							$(this).find("a[name='comment-operate-label']").hide();
						}
					}
				});
			}
			...
			function operate(id) {
				$.post("/rest/um/comment/cross/operate", {
					id: id,
				},
				function(result){
